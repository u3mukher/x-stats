...
1. **First Term:**
   ...
   - \(f_{(x,y)}\): Probability of defensive actions in zone \((x,y)\) leading to successful disruption.
   - \(s_{(x,y)}\): Shot probability from zone \((x,y)\).
...
2. **Second Term:**
   ...
   - \(l_{(x,y)}\): Probability of disrupting the ball from being moved from zone \((x,y)\).
   - \(T_{(x,y)\to(z,w)}\): Transition matrix for moves from zone \((x,y)\) to zone \((z,w)\).
   - \(xT_{(z,w)}\): Expected threat of moving the ball to destination zone \((z,w)\).
