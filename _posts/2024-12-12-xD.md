...
xD<sub>(x,y)</sub> = ( f<sub>(x,y)</sub> ⋅ s<sub>(x,y)</sub> ) + ( l<sub>(x,y)</sub> ⋅ Σ<sub>z=1</sub><sup>Z</sup> Σ<sub>w=1</sub><sup>W</sup> T<sub>(x,y)→(z,w)</sub> ⋅ xT<sub>(z,w)</sub> )
...
1. **First Term:** \( f_{(x,y)} \cdot s_{(x,y)} \)
   ...
   - \(f_{(x,y)}\): Probability of successful disruption of shots.
   - \(s_{(x,y)}\): Shot probability from that zone.
...
2. **Second Term:**
   ...
   - \(l_{(x,y)}\): Probability of disrupting the ball from being moved out of zone \((x,y)\).
   - \(T_{(x,y)\to(z,w)}\): Transition probability from zone \((x,y)\) to zone \((z,w)\).
   - \(xT_{(z,w)}\): Expected threat of zone \((z,w)\).
