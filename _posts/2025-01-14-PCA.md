...
Below is an excerpt of the **loadings** for each metric on each principal component in the **Player PCA**. (Note that we’ve renamed the row headers to be more human-readable.) A positive loading means that metric **increases** along that PC’s axis, while a negative loading means it **decreases**.
...
<table style="border: 1px solid black; border-collapse: collapse;">
  ...
  <!-- ... Additional rows for Shots per 90, Dribbles per 90, etc. -->
</table>
...
**Note**: PC1 is quite strong for players, explaining over 35% of total variance. The loadings suggest it heavily ties to offensive metrics such as *Shots per 90* and *xG per 90*.

---

#### Team PCA: Loadings & Explained Variance

Below is a similar table for the **Team PCA**, focusing on analogous metrics:
...
<table style="border: 1px solid black; border-collapse: collapse;">
  ...
  <!-- ... Additional rows for Team Shots per 90, Team xG per 90, etc. -->
</table>
...
**Note**: For teams, PC1 captures ~46% of total variance and correlates strongly with *Team Possession*, *Team Pass Completion Rate*, *Team xG per 90*, etc.—suggesting an axis of **possession + attacking** style. Lower components highlight more specific traits like long‐ball strategies or counterattacks.

---

### Why Some Metrics Are “Insignificant”

PCA zooms in on the biggest axes of variance in each dataset. Metrics that appear with very small loadings (like xG per 90 in certain components) are overshadowed by stronger signals—maybe certain leagues or clubs differ more in possession and passing length than in xG, so xG doesn’t dominate the early components.
...
If each player has a principal-component coordinate vector \(\mathbf{p} = (p_1, p_2, \ldots, p_{13})\) and each team has \(\mathbf{t} = (t_1, t_2, \ldots, t_{13})\), then I measure their “stylistic distance” simply by **Euclidean distance**:

\[
\text{distance}(\mathbf{p}, \mathbf{t}) 
= \sqrt{ \sum_{i=1}^{13} \bigl(p_i - t_i\bigr)^2 }.
\]
