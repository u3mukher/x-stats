...
<table style="border: 1px solid black; border-collapse: collapse;">
  ...
  <!-- Fill additional rows as needed -->
</table>
...
**Note**: PC1 is quite strong for players, explaining over 35% of total variance. The loadings suggest it heavily ties to *Shots per 90* and *xG per 90*, which indicate a strong attacking emphasis.
...
**Note**: For teams, PC1 captures ~46% of total variance and strongly correlates with *Team Possession*, *Team Pass Completion Rate*, and *Team xG per 90*, indicating a tendency toward a **possession + attacking** style.

---

### Why Some Metrics Are “Insignificant”

PCA focuses on the **largest** axes of variance in each dataset. Metrics that end up with smaller loadings (e.g., xG per 90) are overshadowed by others that vary more widely between players or between teams. This doesn’t mean the smaller metrics have no effect—it just means that in the **top** principal components, their impact is comparatively modest.
...
By looking at each player’s PCA coordinates \(\mathbf{p}\) and each team’s PCA coordinates \(\mathbf{t}\), we can measure their similarity with a straightforward **Euclidean distance** across all principal components:

\[
\text{distance}(\mathbf{p}, \mathbf{t}) 
= \sqrt{(p_1 - t_1)^2 + (p_2 - t_2)^2 + \dots + (p_{13} - t_{13})^2}.
\]

We add up the squared differences between the player and the team in each principal component, and then take the square root of that sum. A **smaller** distance means the player and team align more closely on those underlying style axes, acting as a rough but practical proxy for “fit.”
