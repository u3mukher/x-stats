...
Below is a **complete example** of the **Player PCA** loadings for 13 metrics across 13 principal components, using **bordered HTML tables**. The numeric values here are placeholders—replace them with your actual PCA loadings if desired:
...
<table style="border: 1px solid black; border-collapse: collapse;">
  ...
  <tbody>
    ...
    <!-- ... continue adding rows for Dribbles per 90, Shots per 90, etc. 
         with placeholder or actual loadings 
    -->
    <tr style="border: 1px solid black;">
    ...
    <!-- ... Fill out for xG per 90, Shots per 90, Key Passes per 90, 
         Duel Success Rate, Interceptions per 90, Fouls Committed per 90,
         Counterattacks per 90, Set Piece xG per 90, Dribbles per 90
    -->
  </tbody>
</table>
...
**Note**: Here, PC1 is quite strong for players, explaining over 35% of total variance. The loadings suggest it heavily ties to *Shots per 90* and *xG per 90* (offensive emphasis).

---

#### Team PCA: Loadings & Explained Variance

Below is a similar **bordered table** for the **Team PCA**, again with dummy loadings for illustration:
...
<table style="border: 1px solid black; border-collapse: collapse;">
  ...
  <tbody>
    ...
    <!-- ... continue for Team xG per 90, Team Shots per 90, Team Key Passes per 90, 
         Team Dribbles per 90, Team Set Piece xG per 90, etc.
    -->
  </tbody>
</table>
...
**Note**: For teams, PC1 is even more dominant (~46% variance) and correlates strongly with *Team Possession*, *Team Pass Completion Rate*, and *Team xG per 90*, suggesting a **possession + attacking** dimension.

---

### Why Some Metrics Are “Insignificant”

PCA focuses on the **largest** axes of variance in each dataset. Metrics with low loadings (like xG per 90 in some contexts) are overshadowed by more widely varying metrics. They’re not meaningless, but they don’t drive the *biggest* differences in style.

---

### Comparing Players & Teams for “Fit”

Because we use **two separate** PCA spaces, it’s not a purely algebraic match. However, each space captures *broadly similar* stylistic axes:

- For **players**, one dimension might emphasize “attacking” (shots, xG) vs. a more defensive or possession role.  
- For **teams**, one dimension might emphasize “possession and buildup” vs. “direct/counterattacking.”

If we let \((x,y)\) denote a player–team pair, we define a **distance** \(xD\<sub>(x,y)\</sub>\) that measures how well the player’s style lines up with the team’s style. This distance can combine various factors. For instance, one possible form is:

\[
xD_{(x,y)} \;=\; \bigl(\,f_{(x,y)} \;\cdot\; s_{(x,y)}\bigr)\;+\;\bigl(\,l_{(x,y)} \;\cdot\; \sum_{z}\,\sum_{w}\, T_{(x,y)\to(z,w)} \;\cdot\; xT_{(z,w)}\bigr)
\tag{1}
\]
