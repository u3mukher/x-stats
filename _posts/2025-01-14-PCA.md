...
I run **two separate** PCAs: one on a player-level dataset and one on a team-level dataset. Each set of metrics is thematically similar (e.g., xG, passing, dribbles), so the principal components often capture comparable stylistic ideas even though they’re technically different coordinate spaces. My goal is to uncover these broad stylistic traits and then measure how a given player’s coordinate (in *player PCA space*) compares to a team’s coordinate (in *team PCA space*)—a practical (if not purely algebraic) measure of “fit.”
...
**Note**: PC1 is quite strong for players, explaining over 35% of total variance. The loadings suggest it heavily ties to *Shots per 90* and *xG per 90*, indicating a strong attacking emphasis.
...
**Note**: For teams, PC1 captures ~46% of total variance and correlates strongly with *Team Possession*, *Team Pass Completion Rate*, *Team xG per 90*, etc.—indicating a **possession + attacking** style. Lower components highlight more specific traits, such as long-ball strategies or heavy counterattacks.

---

### Why Some Metrics Are “Insignificant”

PCA focuses on the **largest** axes of variance in each dataset. Metrics that end up with smaller loadings (e.g., xG per 90 in some components) are overshadowed by others that vary more widely among players or teams. This doesn’t mean those smaller metrics have no effect—only that, in the **top** principal components, their impact is more modest.
...
Here, each (p<sub>i</sub>-t<sub>i</sub>) measures how differently the player and team score on the i-th principal component, and **smaller** distances imply a closer stylistic match. **This approach is a *heuristic*.** Because the PCAs are run **separately** for players and teams, we rely on the assumption that both groups share *conceptually similar axes* (like “attacking vs. conservative” or “possession vs. direct”). When we look at the **loadings** above, we see **players**’ PC1 strongly tied to Shots/xG, while **teams**’ PC1 ties to Shots, xG, and Possession. This parallel emphasis on offensive metrics suggests the main dimensions **do** align conceptually (i.e., PC1 in each dataset captures an “attacking” style). Hence, **teams** that rank “closer” to a given **player** often match that player’s style in real-life scenarios (e.g., successful transfers), which validates our assumption in practice.
...
## Interpreting the Side‐by‐Side PCA Plots

We now have **two separate** PCA scatterplots placed side by side:
...
- **Left Subplot (Player PCA):** Each player’s style is mapped onto two axes:
  ...
  - **PC2 (Defensive Engagement + Forward Passing):** Players higher on this axis perform more interceptions/duels and also attempt more progressive passes, while those lower do less of both.
...
- **Player PCA Coordinates:**  
  \(\text{PC1} \approx -1.397,\quad \text{PC2} \approx 0.132\)

  McAuley sits slightly on the **negative** side of the Attacking axis (PC1), indicating a lower xG/shot contribution. A moderate **positive** PC2 suggests he has a fair amount of **defensive engagement** (interceptions, duel success) plus some forward‐passing involvement.

- **Team PCA Coordinates (West Bromwich Albion):**  
  \(\text{PC1} \approx -3.836,\quad \text{PC2} \approx 0.004\)

  West Brom ranks **significantly left** on the Team Attack + Possession axis, consistent with a more direct, less possession‐focused approach. Near‐zero on Team PC2 implies neither especially high nor especially low dribble/defensive emphasis.

Together, McAuley’s own defensive yet somewhat direct style correlates with West Brom’s overall strategy—less possession, more direct transitions, and moderate defensive involvement.

---

### 2) Barcelona

- **Team PCA Coordinates:**  
  \(\text{PC1} \approx 5.272,\quad \text{PC2} \approx -0.028\)

  A **strongly positive** PC1 signals a high xG/possession approach, characteristic of Barcelona’s well‐known attacking style. Near‐zero on PC2 places them around the middle in terms of dribbles/defensive actions, but their emphasis on controlled, high‐possession buildup is the standout trait.

**Piqué (CB: BAR)** sits in the **Player PCA** at:
\[
\text{PC1} \approx -0.498,\quad \text{PC2} \approx 1.270,
\]
which favors a comfortable‐on‐the‐ball role, emphasizing progressive passing and a measured defensive presence. Conceptually, he mirrors Barcelona’s broader philosophy: high possession, deliberate buildup, and sustained attacking control.

---

### Observations & Caveats

1. **Tactical Alignment**: McAuley and West Brom appear on the “direct, less attacking‐possession” end in their respective PCAs, indicating consistency between an individual defender’s style and the team’s overall approach. Meanwhile, Piqué and Barcelona align on the opposite end—higher attacking/possession.  

2. **Two Separate Spaces**: Each subplot uses distinct principal components because players and teams require different metrics. So you cannot directly compare a player’s coordinate (e.g., PC1=–1.397) to a team’s coordinate in the other plot.  

3. **No Position‐Based Grouping**: These PCAs do not explicitly cluster by position. Hence, contrasting defenders like Piqué and McAuley appear far apart if their styles differ significantly. A future breakdown could incorporate position‐specific data for more nuanced insights.

Despite these limitations, viewing **players** and **teams** side by side clearly shows how attacking/possession or direct/defensive traits manifest at both levels, reinforcing the synergy between an individual’s style and the tactical identity of their club.
