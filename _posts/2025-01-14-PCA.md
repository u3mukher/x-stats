...
For the sake of clarity, we only plot the first two principal components for both the player and team PCAs. These are represented by the **two separate** PCA scatterplots placed side by side above.
...
### Why Separate PCAs—and Validating This Approach

We run **two distinct PCAs**—one for players, one for teams—because **individual** playing style and **collective** team style involve different data and variance patterns. By plotting these separate spaces side by side, we can still compare **where** each player and team stand on their respective axes, then see how well they match in practice. 
