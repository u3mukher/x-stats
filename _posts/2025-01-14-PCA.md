...
To measure similarity between a player’s PCA coordinates \(\mathbf{p} = (p_1,\dots,p_{13})\) and a team’s PCA coordinates \(\mathbf{t} = (t_1,\dots,t_{13})\), I use this distance function:

= √( (p<sub>1</sub> - t<sub>1</sub>)² + (p<sub>2</sub> - t<sub>2</sub>)² + … + (p<sub>13</sub> - t<sub>13</sub>)² )


Here, each \((p_i - t_i)\) measures how differently the player and team score on the \(i\)-th principal component, and **smaller** distances imply a closer stylistic match. Despite the two PCAs being separate, if they capture broadly similar ideas (e.g., attacking vs. defensive, possession vs. direct), then teams that lie “close” to a player often turn out to be better real-world fits.



In plainer words, the term \(f_{(x,y)} \cdot s_{(x,y)}\) might represent a **direct** similarity factor (e.g., how close the player’s shooting rate is to the team’s style), while the summation \(\sum_{z}\,\sum_{w}\,\dots\) could fold in more *positional* or *possession* aspects. The exact meaning of these symbols depends on your model. But the core idea is that a **smaller** \(xD\<sub>(x,y)\</sub>\) implies the player and team align more closely on fundamental style dimensions, gleaned from the PCA.

This approach is a **heuristic**. Because the PCAs are run **separately** for players and teams, we rely on the assumption that both groups share *conceptually similar axes* (like “attacking vs. conservative” or “possession vs. direct”). When that assumption holds, **teams** that rank “closer” to a given **player** often match that player’s style in real-life scenarios (e.g., successful transfers).
